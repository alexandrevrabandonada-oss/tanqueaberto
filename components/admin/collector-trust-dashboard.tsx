...
function getTrustVisual(stage: string) {
  const value = stage.toLowerCase();

  if (["very_trusted", "trusted", "muito_confiável", "confiável"].includes(value)) {
    return {
      badge: (
  ...
  if (["review_needed", "em_revisão"].includes(value)) {
    return {
      badge: (
      ...
      <div className="overflow-hidden rounded-[24px] border border-white/5 bg-black/20">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {filtered.map((c) => {
              const trust = getTrustVisual(c.trust_stage);
              ...
              return (
                <tr key={`${c.nickname}:${c.ip_hash}`} className="transition hover:bg-white/2">
                  ...
                  <td className="px-6 py-4">{trust.badge}</td>
