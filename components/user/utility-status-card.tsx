...
export function UtilityStatusCard({ unreadCount = 0 }: UtilityStatusCardProps) {
  ...
  if (!identity.isLoaded) {
    return <div className="h-32 w-full animate-pulse rounded-[28px] border border-white/10 bg-white/5" />;
  }

  const status = identity.utilityStatus;
    ...
    <div className="relative overflow-hidden rounded-[32px] border border-white/12 bg-black/40 p-5 shadow-2xl backdrop-blur-xl">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="relative">
            <div className="text-right">
              <p className="text-[10px] uppercase font-bold text-white/30 tracking-widest">Seu ritmo</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-3">
          <p className="text-[10px] font-black uppercase text-white/30 tracking-widest mb-1">Seu impacto</p>
          <div className="flex items-center gap-2 text-white">
            <CheckCircle2 className="h-3 w-3 text-green-500" />
            <span className="text-xs font-bold">{trust?.approvedReports || 0} Preços aprovados</span>
          </div>
        </div>

        <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-3">
          <p className="text-[10px] font-black uppercase text-white/30 tracking-widest mb-1">Próximo passo</p>
      ...
      <div className="mt-4 flex items-center gap-3 rounded-2xl bg-[color:var(--color-accent)]/10 border border-[color:var(--color-accent)]/20 p-3">
        ...
        <div className="flex-1">
          <p className="text-[10px] font-black uppercase text-[color:var(--color-accent)] tracking-widest">Agora</p>
