...
import { Zap, RefreshCcw, CheckCircle2, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { SectionCard } from "@/components/ui/section-card";
import { Badge } from "@/components/ui/badge";
import { loadSubmissionQueue, markEntryAsSuccess, getSubmissionQueueFuelEntries, type SubmissionQueueEntry } from "@/lib/queue/submission-queue";
...
export function QueueAssistant({ onQueueUpdated, onReviewRequested }: QueueAssistantProps) {
  ...
  useEffect(() => {
    const refreshQueue = async () => {
      ...
      setItems(queue.filter((item) => item.status !== "success" && item.status !== "expired"));
    };
  ...
  const flushAll = async () => {
    ...
    setIsFlushing(true);
    const readyItems = items.filter((item) => item.hasPhoto);
    ...
    for (let index = 0; index < readyItems.length; index += 1) {
      ...
      if (!photo) {
        setProgress((current) => ({ ...current, fail: current.fail + 1 }));
        continue;
      }
      ...
      formData.append("priceEntriesJson", JSON.stringify(entries));
      ...
      try {
        ...
        if (result.success) {
          ...
          setProgress((current) => ({ ...current, success: current.success + 1 }));
        } else {
          setProgress((current) => ({ ...current, fail: current.fail + 1 }));
        }
      } catch {
    ...
    setItems(updatedQueue.filter((item) => item.status !== "success" && item.status !== "expired"));
  ...
  if (!isOnline && items.length > 0) {
    return (
      <div className="fixed bottom-24 left-4 right-4 z-50 animate-in fade-in slide-in-from-bottom-4 sm:left-auto sm:right-6 sm:w-80">
        <SectionCard className="border-white/10 bg-black/80 p-4 shadow-2xl backdrop-blur-xl">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-[11px] leading-relaxed text-white/50">Aguardando conexao para reenviar automaticamente.</p>
            </div>
          </div>
        </SectionCard>
      </div>
  ...
  if (isOnline && items.length > 0 && !isFlushing) {
    return (
      <div className="fixed bottom-24 left-4 right-4 z-50 animate-in fade-in slide-in-from-bottom-4 sm:left-auto sm:right-6 sm:w-96">
        <SectionCard className="border-[color:var(--color-accent)]/30 bg-black/80 p-4 shadow-2xl backdrop-blur-xl">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold uppercase tracking-widest text-[color:var(--color-accent)]">Conexao Estabelecida</p>
                <Badge variant="accent" className="h-5 px-1.5 text-[9px]">{items.length} Pendente{items.length > 1 ? "s" : ""}</Badge>
              </div>
              <p className="mt-1 text-sm font-medium text-white">Voce tem pendencias locais</p>
  ...
  if (isFlushing) {
    return (
      <div className="fixed bottom-24 left-4 right-4 z-50 animate-in fade-in slide-in-from-bottom-4 sm:left-auto sm:right-6 sm:w-96">
        <SectionCard className="border-white/10 bg-black/90 p-5 shadow-2xl backdrop-blur-xl">
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="mt-2 text-lg font-semibold text-white">
              {progress.current < progress.total ? `Enviando ${progress.current}/${progress.total}...` : "Processamento concluido"}
            </h3>
