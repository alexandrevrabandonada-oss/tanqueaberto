...
select
  pr.station_id,
  pr.fuel_type,
  (timezone('America/Sao_Paulo', pr.reported_at))::date as day,
...
group by pr.station_id, pr.fuel_type, (timezone('America/Sao_Paulo', pr.reported_at))::date;
...
where pr.status = 'approved'
  and s.is_active = true
group by s.city, pr.fuel_type, (timezone('America/Sao_Paulo', pr.reported_at))::date;
